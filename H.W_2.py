...
def benchmark(array, repetitions):
    ...
    magic_array = random.randint(MIN_MAGIC_NUMBER,MAX_MAGIC_NUMBER + 1, \
    size = (repetitions))
    # measure time spent for calculations
    dtStart = datetime.datetime.now()
    while step >= 0:
        position, attempt = binarySearch(array, magic_array[step - 1],\
        0, len(array) - 1)
        if position == None:
            raise Exception(f'Something goes terrible wrong.'
            f'The magic number {magic_array[step - 1]}'
            f'was not found in the array set')
        # save attempt result
...
if position != None:
    ...
    print(f'К сожалению загаданное по условию число \'{magic_number}\'\
    не удалось обнаружить в массиве вариантов\n')
